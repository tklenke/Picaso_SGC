...
//        #define PIC_DISPLAY_MODE_PAGE_WRITE        0x0B
    ...
    //cFlag masks
    //pen 1000 0000
    //transparent opague text 0100 0000
    #define PEN_MASK 0x80
    #define TRANSPARENT_OPAQUE_MASK 0x40 
...
/*******************************************************
    ...
    Description:
    Params:
        x, y, x2, width, height, not_filled (0|1) and RGB color
        i.e. not_filled = 1 then wire frame, 
    Return: 1 if succesful or 0 if not
 ********************************************************/
uint8_t PicasoSGC::DrawRectangle(unsigned int x, unsigned int y, unsigned int width, unsigned int height, char wire, uint8_t red, uint8_t green, uint8_t blue)
{
    unsigned int x2, y2;
    
    if ( wire != ((cFlags & PEN_MASK) >> 7) )
    {
        if (!SetPenSize( wire ))
            return 0;
    }
    ...
    sSerial->write(x & 0xFF);        // LSB    

    sSerial->write(y >> 8);            // MSB
    sSerial->write(y & 0xFF);        // LSB    
